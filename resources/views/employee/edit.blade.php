                ...
                <div class="card emp_details">
                    ...
                    <div class="card-body employee-detail-edit-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-6" id="deployed_at_wrap" style="display:none;">
                                {!! Form::label('deployed_at', __('Deployed At'), ['class' => 'form-label']) !!}
                                {!! Form::text('deployed_at', old('deployed_at', $employee->deployed_at), [
                                    'class' => 'form-control',
                                    'placeholder' => __('Enter deployment location'),
                                ]) !!}
                            </div>

                            <div class="form-group col-md-6" id="deployed_date_wrap" style="display:none;">
                                {!! Form::label('deployed_date', __('Deployed Date'), ['class' => 'form-label']) !!}
                                {!! Form::date('deployed_date', old('deployed_date', optional($employee->deployed_date)->format('Y-m-d')), [
                                    'class' => 'form-control',
                                    'autocomplete' => 'off',
                                    'placeholder' => __('Select deployment date'),
                                ]) !!}
                            </div>
...
@push('script-page')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const atWrap = document.getElementById('deployed_at_wrap');
            const dateWrap = document.getElementById('deployed_date_wrap');
            const atInput = document.querySelector('input[name="deployed_at"]');
            const dateInput = document.querySelector('input[name="deployed_date"]');

            // If you passed a known GUARD ID from the controller, you can use ID-based check:
            const GUARD_ID = "{{ $guardDesignation->id ?? '' }}";

            function isGuardSelected() {
                if (GUARD_ID) {
                    return designationSelect.value == String(GUARD_ID);
                }
                const text = (designationSelect.options[designationSelect.selectedIndex]?.text || '')
                    .trim().toLowerCase();
                return text.includes('guard') || text.includes('gaurd');
            }

            function toggleDeployedFields() {
                const on = isGuardSelected();
                atWrap.style.display = on ? '' : 'none';
                dateWrap.style.display = on ? '' : 'none';

                if (on) {
                    atInput.setAttribute('required', 'required');
                    dateInput.setAttribute('required', 'required');
                } else {
                    atInput.removeAttribute('required');
                    dateInput.removeAttribute('required');
                }
            }

            designationSelect.addEventListener('change', toggleDeployedFields);
            toggleDeployedFields(); // initialize on page load for existing employee
        });
    </script>
