    ...
    <div class="float-end">
        ...
        @can('create contract')
            @if (empty($deal->contract_id))
                <a href="#" data-size="lg" data-url="{{ route('deals.convert.contract', $deal->id) }}"
                    data-ajax-popup="true" data-bs-toggle="tooltip" title="{{ __('Convert to Contract') }}"
                    class="btn btn-sm btn-success">
                    <i class="ti ti-file-text"></i>
                </a>
            @else
                <a href="{{ route('contract.show', $deal->contract_id) }}" data-bs-toggle="tooltip"
                    title="{{ __('View Contract') }}" class="btn btn-sm btn-info">
...
@section('content')
    @php
        $customData = App\Models\CustomField::getData($deal, 'deal');
    @endphp
    <div class="row">
        <div class="col-sm-12">
            <div class="row">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row" style="row-gap: 20px">
                                @foreach ($customData as $key => $data)
                                    @php
                                        $field = App\Models\CustomField::where('id', $key)->first();
                                    @endphp
                                    <div class="col-md-4 col-sm-4">
                                        <div class="d-flex align-items-start">
                                            <div class="ms-2">
                                                <p class="text-muted text-sm mb-0">{{ $field->name }}</p>
                                                <h5 class="mb-0 text-primary">{{ !empty($data) ? $data : '' }}</h5>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
