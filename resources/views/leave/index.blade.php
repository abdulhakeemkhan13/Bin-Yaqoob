...
@section('content')
    ...
    <div class="col-xl-12">
            ...
            <div class="card-body table-border-style">
                    ...
                    <table class="table datatable" id="leave-table">
                            <thead>
                            <tr>
                            ...
                            @foreach ($leaves as $leave)
                                <tr data-row_id="{{$leave->id}}">
                                    @if(\Auth::user()->type!='Employee')
                                        <td>@if($leave->updated_at < \Carbon\Carbon::now()->subHours(24) && $leave->status=="Pending")
                                                <small style="position: absolute; position: absolute; margin-top: -16px; "><span class="badge bg-danger" data-bs-toggle="tooltip" title="{{ __('No Activity for the Last 24 Hours ') }}"> {{ __('Follow Up') }}</span></small></a>
                                            @endif
                                            {{ !empty($leave->employees) ? $leave->employees->name : '-'}}
                                        </td>
                                    @endif
                                    ...
                                    <td>
                                        ...
                                        <div class="action-btn bg-danger ms-2">
                                            ...
                                            <a href="#" class="mx-3 btn btn-sm  align-items-center bs-pass-para" data-bs-toggle="tooltip" title="{{__('Delete')}}" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$leave->id}}').submit();" data-ajax="true">
