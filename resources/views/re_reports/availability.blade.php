@extends('re_reports.layout', ['title' => __('Project/ Block/ Unit Availability Report')])
    ...
    @foreach ($projects as $project)
        <div class="project-section mb-5">
            ...
            @foreach ($project->towers as $tower)
                @php
                    $hasUnits =
                        $tower->floors->reduce(function ($carry, $floor) {
                            return $carry + $floor->units->count();
                        }, 0) > 0;
                @endphp
                @if ($hasUnits)
                    <div class="tower-section ms-2 mb-4">
                        <h6 class="bg-info text-white p-2">{{ __('Block / Building') }}: {{ $tower->tower_name }}</h6>
