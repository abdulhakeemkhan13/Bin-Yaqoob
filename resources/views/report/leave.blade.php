...
@push('script-page')
    ...
    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12 date-picker d-none">
        <div class="btn-box">
            {{ Form::label('date', __('Date'),['class'=>'form-label']) }}
            {{-- This uses the standard HTML5 date input which provides a calendar UI --}}
...
<script>
    ...
    $(document).ready(function() {
        // Event listener for when the 'type' radio button changes
        $('input[name="type"]:radio').on('change', function (e) {
            ...
            if (type === 'monthly') {
                ...
                // Hide the Daily Date Picker
                $('.date-picker').removeClass('d-block').addClass('d-none');
                // Ensure the Year dropdown is hidden (if still present)
                $('.year').removeClass('d-block').addClass('d-none'); 

            } else if (type === 'daily') {
                // When 'Daily' is selected:
                // Show the Daily Date Picker
                $('.date-picker').removeClass('d-none').addClass('d-block');
        ...
        // Trigger the change event on load to set the correct initial visibility
