...
@push('script-page')
    ...
    $(".generate_button").click(function(){
        ...
        // 1. Validate From Date is not empty
        if(from_date == ''){
        ...
        // 2. Validate To Date is not empty
        if(to_date == ''){
            $("#toDate").text("Please select date");
            toDateValid = false;
        }else{
            // Clear any previous error before checking date order
            $("#toDate").empty();
        }
        
        // 3. **New Logic:** Check if To Date is before From Date (only if both are selected)
        if(fromDateValid && toDateValid) {
            ...
            if(toDateObj < fromDateObj) {
                // To Date is before From Date - Show error on To Date
                $("#toDate").text("To Date must be on or after From Date");
                toDateValid = false; // Prevent AJAX call
            } else {
        ...
        // 4. Proceed with AJAX call only if all validations pass
        if(fromDateValid && toDateValid) {
            $.ajax({
