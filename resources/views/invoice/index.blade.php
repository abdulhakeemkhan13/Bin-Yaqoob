...
{{--                                @endif--}}
                            ...
                            @foreach ($invoices as $invoice)
                                <tr>
                                    <td class="Id">
                                        <a href="{{ route('invoice.show', \Crypt::encrypt($invoice->id)) }}" style="padding-top: 11px;" class="btn btn-outline-primary">{{ AUth::user()->invoiceNumberFormat($invoice->invoice_id) }}
                                            @if($invoice->updated_at < \Carbon\Carbon::now()->subHours(96) && $invoice->status != 4)
                                                <small style="position: absolute; position: absolute; margin-top: -15px; margin-left: -24px;"><span class="badge bg-danger" data-bs-toggle="tooltip" title="{{ __('No Activity for the Last 96 Hours ') }}"> {{ __('Follow Up') }}</span></small></a>
                                            @endif
                                    </td>
