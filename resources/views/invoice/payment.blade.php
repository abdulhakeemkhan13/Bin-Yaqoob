...
<div class="modal-body">
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            {{ Form::number('amount', $invoice->getDue(), ['class' => 'form-control', 'required' => 'required', 'step' => '0.01', 'placeholder' => __('Enter Amount')]) }}
        </div>
        ...
        <div class="form-group col-md-6" id="title-of-account-wrap" style="display:none;">
            ...
            {{ Form::text('title_of_account', '', ['id' => 'title_of_account', 'class' => 'form-control', 'placeholder' => __('Enter cheque holder name')]) }}
            <small class="text-muted">{{ __('Required when Type is Cheque') }}</small>
        </div>
    </div>
...
(function () {
  function toggleCheque() {
    ...
    var val = (typeEl.value || "").toLowerCase();
    var isCheque = (val === "cheque");
  ...
  // If using Bootstrap modals loaded via AJAX
  document.addEventListener("shown.bs.modal", toggleCheque);

  // React to changes (native select)
  document.addEventListener("change", function (e) {
    if (e.target && e.target.id === "type") toggleCheque();
  });

  // React to Select2 selection if used
