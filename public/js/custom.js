...
// 2) Then in your custom.js, before any $.ajax calls:
$.ajaxSetup({
    headers: {
  ...
  $(document).on("click", ".bs-pass-para", function () {
    ...
    var card = form.closest("[data-row_id]"); // Add this to find card elements
    var rowId = card.length ? card.attr("data-row_id") : null; // Get the row_id if it exists
    ...
    swalWithBootstrapButtons.fire({
      ...
      $.ajax({
        ...
        success: function(response) {
          // Handle both tr rows and card elements
          if (row.length) {
          ...
          // If we have a response with card data, use it
          if (response && response.data && response.data.card_id) {
            // If there's a handler for card deletion
            if (typeof window.ajaxHandler !== 'undefined' && window.ajaxHandler.deleteCard) {
              window.ajaxHandler.deleteCard(response.data.card_id);
            }
          }
...
// Add this to your document ready function or wherever you initialize modals
// $(document).on('show.bs.modal', '.modal', function (e) {
//     console.log('Modal opened');
//     // Check all forms in the modal
//     $(this).find('form').each(function() {
//         console.log('Form in modal:', this);
