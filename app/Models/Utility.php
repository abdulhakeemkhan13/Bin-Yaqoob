...
//    }
    ...
    public static function employeePayslipDetail($employeeId, $month)
    {
        ...
        public static function leave_deduction($employeeId, $month)
    {
        $deduction['leaves'] = PaySlip::where('employee_id', $employeeId)->where('salary_month', $month)->get();

        $employess = Employee::find($employeeId);

        $leave_deduction =0;
        $leave_late =0;
        $arrayJson = json_decode($deduction['leaves']);
        foreach ($arrayJson as $leaves) {
            $leave = json_decode($leaves->leaves);
            $totalDays = date('t', strtotime($leaves->salary_month . '-01'));
            $perday = $leaves->basic_salary / $totalDays;
            $leave_deduction = $perday * @$leave->unpaid;
            $leave_late = $perday * @$leave->late;

        }
        return $leave_deduction + $leave_late;
    }
...
//                    dd(\Storage::disk(),$path);
    ...
    public static function addTransactionLines($data, $action)
    {
        ...
        $transactionLines->created_by = Auth::user()->creatorId();
        $transactionLines->save();
        $transactionLines->created_at = $data['created_at'];
        $transactionLines->updated_at = $data['created_at'];
        $transactionLines->save();
    }
    ...
    public static function jrentry($data)
    {
        ...
        try {
            ...
            $journal->customer_id = $data['customer_id'];
    ...
    public static function brv_entry($data)
    {
        ...
        $journal->journal_id = $latest;
        $journal->date = $data['date'];
        $journal->vendor_id = $data['vendor_id'] ?? null;
        $journal->customer_id = $data['customer_id'] ?? null;
        $journal->employee_id = $data['employee_id'] ?? null;
        $journal->reference = $data['reference'] ?? '';
        $journal->description = 'Receipt on Invoice No : ' . @$data['no'];
        $journal->reference_id = $data['id'];
        $journal->category = $data['category'];
    ...
    public static function bpv_entry($data)
    {
        ...
        $journal->journal_id = $latest;
        $journal->date = $data['date'];
        $journal->vendor_id = $data['vendor_id'];
        $journal->customer_id = $data['customer_id'];
