...
class AllowanceOptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create allowance option')) {
            ...
            $html = view('allowanceoption.appendrow', compact('allowanceoption'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "allowanceoption-table",
                'action' => 'add',
                'row_id' => $allowanceoption->id,
            ];
            return response()->json(['success' => true, 'message' => __('AllowanceOption  successfully created.'), 'data' => $data]);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, AllowanceOption $allowanceoption)
    {
        ...
        if (\Auth::user()->can('edit allowance option')) {
            if ($allowanceoption->created_by == \Auth::user()->creatorId()) {
                ...
                return response()->json(['success' => true, 'message' => __('AllowanceOption successfully updated.'), 'data' => $data]);
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function destroy(AllowanceOption $allowanceoption)
    {
        if (\Auth::user()->can('delete allowance option')) {
            if ($allowanceoption->created_by == \Auth::user()->creatorId()) {
                Utility::makeActivityLog(\Auth::user()->id, 'AllowanceOption', $allowanceoption->id, 'delete AllowanceOption', $allowanceoption->name);
