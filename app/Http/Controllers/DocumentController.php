...
class DocumentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create document type'))
        {
            ...
            $html = view('document.appendrow', compact('document'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "document-table",
                'action' => 'add',
                'row_id' => $document->id,
            ];
            return response()->json(['success' => true, 'message' => __('Document type successfully created.'), 'data' => $data]);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, Document $document)
    {
        ...
        if(\Auth::user()->can('edit document type'))
        {
            if($document->created_by == \Auth::user()->creatorId())
            {
                ...
                return response()->json(['success' => true, 'message' => __('Document type successfully updated.'), 'data' => $data]);
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
