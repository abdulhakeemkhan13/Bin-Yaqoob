...
class TrainingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $training->status = 0; // Set default status to 'Pending'
            $training->created_by = \Auth::user()->creatorId();
            $training->owned_by = \Auth::user()->ownedId();
            $training->save();

            if($training->employee_id){
                $emp = Employee::find($training->employee_id);
                $emp->termination_date = $training->end_date;
                $emp->save();
            }
            ...
            // Get the status array from the Training model
            $status = Training::$Status;
            
            $html = view('training.appendrow', compact('training', 'status'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "training-table",
                'action' => 'add',
                'row_id' => $training->id,
            ];
            ...
            return response()->json([
                'success' => true, 
                'message' => __('Training successfully created.'), 
                'data' => $data
            ]);
        } catch (\Exception $e) {
            DB::rollback();
            \Log::error('Training creation error: ' . $e->getMessage());
    ...
    public function update(Request $request, Training $training)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true, 
                'message' => __('Training successfully updated.'), 
                'data' => $data
            ]);
        } catch (\Exception $e) {
            DB::rollback();
