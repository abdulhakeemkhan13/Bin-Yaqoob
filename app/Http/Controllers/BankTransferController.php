...
class BankTransferController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create bank transfer'))
        {
            ...
            $html = view('bank-transfer.appendrow', ['transfer' => $transfer])->render();
            $data = [
                'datarow' => $html,
                'table_id' => "bank-transfer-table",
                'action' => 'add',
                'row_id' => $transfer->id,
            ];
            return response()->json(['success' => true, 'message' => __('Amount successfully transfer.'), 'data' => $data]);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, BankTransfer $transfer,$id)
    {
        ...
        if(\Auth::user()->can('edit bank transfer'))
        {
            ...
            return response()->json(['success' => true, 'message' => __('Amount successfully transfer updated.'), 'data' => $data]);;
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
