...
class JobApplicationController extends Controller
{
    ...
    //             $containerId = 'task-list-' . $job->stage; // <-- maps to the real DOM id

    //             $cardHtml = view('jobApplication.appendrowJobApplication', [
    //                 'application' => $job,
    ...
    //             $data = [
    //                 'datarow'        => $cardHtml,
    //                 // Support both possible keys so your global handler (table vs card) can use either:
    //                 'table_id'       => $containerId,          // legacy handlers often look for this
    //                 'card_container' => $containerId,          // if your handler looks for this
    //                 'action'         => 'add',
    //                 'row_id'         => $job->id,
    //             ];
    ...
    //                 'success' => true,
    //                 'message' => __('Job application successfully created.'),
    //                 'data'    => $data,
    //             ]);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!$usr->can('create job application')) {
            ...
            // Resolve default stage (like default pipeline in your Lead code)
            $column  = ($usr->type === 'company') ? 'created_by' : 'owned_by';
            $ownerId = ($usr->type === 'company') ? $usr->creatorId() : $usr->ownedId();

            // Prefer stages scoped to current owner/company; fall back to creator
            $stage = JobStage::where($column, $ownerId)->orderBy('order', 'asc')->first();
            if (!$stage) {
                $stage = JobStage::where('created_by', $usr->creatorId())->orderBy('order', 'asc')->first();
            }
            if (empty($stage)) {
                return response()->json(['error' => __('Please create at least one Job Stage first.')], 422);
            }

            // Uploads (keep your Utility::upload_file)
            $profileFileName = '';
            if (!empty($request->profile)) {
                $filenameWithExt = $request->file('profile')->getClientOriginalName();
                $filename        = pathinfo($filenameWithExt, PATHINFO_FILENAME);
                ...
                $dir  = 'uploads/job/profile';
                $path = \App\Models\Utility::upload_file($request, 'profile', $profileFileName, $dir, []);
                if (!isset($path['flag']) || $path['flag'] != 1) {
                    return response()->json(['error' => __($path['msg'] ?? 'Profile upload failed.')], 422);
                }
            }
            ...
            if (!empty($request->resume)) {
                ...
                $filename1        = pathinfo($filenameWithExt1, PATHINFO_FILENAME);
                $extension1       = $request->file('resume')->getClientOriginalExtension();
                $resumeFileName   = $filename1 . '_' . time() . '.' . $extension1;

                $dir  = 'uploads/job/resume';
                $path = \App\Models\Utility::upload_file($request, 'resume', $resumeFileName, $dir, []);
                if (!isset($path['flag']) || $path['flag'] != 1) {
                    return response()->json(['error' => __($path['msg'] ?? 'Resume upload failed.')], 422);
                }
            }
            ...
            $application->profile         = $profileFileName;
            ...
            $application->custom_question = json_encode($request->question ?? []);
            $application->created_by      = $usr->creatorId();
            $application->owned_by        = $usr->ownedId();
            $application->save();
            ...
            // RENDER the card partial (pass what the partial uses)
            $cardHtml = view('jobApplication.appendrowJobApplication', [
            ...
            // This MUST match your stage column DOM id: id="task-list-{stage_id}"
            $containerId = 'task-list-' . $application->stage;

            // Shape the payload like your Lead example
            $data = [
                'card_html'      => $cardHtml,
                'card_container' => $containerId, // where to append
                'action'         => 'add',
    ...
    public function destroy(JobApplication $jobApplication)
    {
        if (\Auth::user()->can('delete job application')) {
            Utility::makeActivityLog(\Auth::user()->id, 'Job application', $jobApplication->id, 'delete Job application', $jobApplication->name);
    ...
    public function jobBoardStore(Request $request, $id)
    {
        ...
        try {
            ...
            $html = view('jobApplication.Appendrow', compact('jobBoard'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "job-onboard-table",
                'action' => 'add',
                'row_id' => $jobBoard->id,
            ];
            return response()->json(['success' => true, 'message' => __('Candidate succefully added in job board.'), 'data' => $data]);
    ...
    public function jobBoardUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            return response()->json(['success' => true, 'message' => __('Job board Candidate succefully updated.'), 'data' => $data]);
