...
class LoanOptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create loan option')) {
            ...
            $html = view('loanoption.appendrow', compact('loanoption'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "loanoption-table",
                'action' => 'add',
                'row_id' => $loanoption->id,
            ];
            return response()->json(['success' => true, 'message' => __('LoanOption successfully created.'), 'data' => $data]);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, LoanOption $loanoption)
    {
        ...
        if (\Auth::user()->can('edit loan option')) {
            if ($loanoption->created_by == \Auth::user()->creatorId()) {
                ...
                return response()->json(['success' => true, 'message' => __('LoanOption successfully updated.'), 'data' => $data]);
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
