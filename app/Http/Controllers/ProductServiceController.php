...
class ProductServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create product & service')) {
            ...
            \DB::commit();
            $html = view('productservice.appendrow', compact('productService'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "productservice-table",
                'action' => 'add',
                'row_id' => $productService->id,
            ];
            return response()->json(['success' => true, 'message' => __('Product & Service successfully created.'), 'data' => $data]);
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (\Auth::user()->can('edit product & service')) {
            ...
            if ($productService->created_by == \Auth::user()->creatorId()) {
                ...
                return response()->json(['success' => true, 'message' => __('Product & Service successfully updated.'), 'data' => $data]);
            } else {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        } else {
