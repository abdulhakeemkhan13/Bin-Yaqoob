    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($setings['resignation_sent'] == 1)
            {
                ...
                \DB::commit();
                Utility::makeActivityLog(\Auth::user()->id, 'Resignation', $resignation->id, 'Create Resignation', $resignation->name);
                
                if($request->ajax())
                {
                    $html = view('resignation.appendrow', compact('resignation'))->render();
                    $data = [
                        'datarow' => $html,
                        'table_id' => "resignation-table",
                        'action' => 'add',
                        'row_id' => $resignation->id,
                    ];
                    return response()->json(['success' => true, 'message' => __('Resignation successfully created.'), 'data' => $data]);
                }
    ...
    public function update(Request $request, Resignation $resignation)
    {
        ...
        if(\Auth::user()->can('edit resignation'))
        {
            if($resignation->created_by == \Auth::user()->creatorId())
            {
                ...
                return response()->json(['success' => true, 'message' => __('Resignation successfully updated.'), 'data' => $data]);
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
