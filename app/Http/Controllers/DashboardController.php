        ...
        if (Auth::check()) {
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                    ...
                    $data['upcomingDeadlines'] = Reminder::with('type')
                        ->where('user_id', \Auth::id())
                        ->where('is_completed', false)
                        ->whereDate('remind_from_date', '<=', $today)  // start showing when window opens
                        ->whereDate('due_date', '>=', $today)          // stop showing once overdue
