    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create termination'))
        {
            ...
            if($termination){
                ...
                $emp->termination_date = $termination->termination_date;
                $emp->save();
            }
...
//                $termination->email = $employee->email;
                ...
                if($request->ajax()) {
                    $html = view('termination.appendrow', compact('termination'))->render();
                    $data = [
                        'datarow' => $html,
                        'table_id' => "termination-table",
                        'action' => 'add',
                        'row_id' => $termination->id,
                    ];
                    return response()->json(['success' => true, 'message' => __('Termination  successfully created.'), 'data' => $data]);
                } else {
    ...
    public function update(Request $request, Termination $termination)
    {
        ...
        try {
            if(\Auth::user()->can('edit termination'))
            {
                ...
                return response()->json(['success' => true, 'message' => __('Termination successfully updated.'), 'data' => $data]);
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
