...
class ProjectReportController extends Controller
{
        ...
        public function index(Request $request)
    {
        ...
        // Get the last task stage or create a default one if none exists
        $last_task = TaskStage::orderBy('order', 'DESC')
            ->where('created_by', \Auth::user()->creatorId())
            ->first();

        if (!$last_task) {
            // Create a default task stage if none exists
            $last_task = TaskStage::create([
                'name' => 'Completed',
                'order' => 1,
