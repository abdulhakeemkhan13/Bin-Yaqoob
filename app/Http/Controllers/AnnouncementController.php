<?php

namespace App\Http\Controllers;

use App\Models\Announcement;
use App\Models\AnnouncementEmployee;
use App\Models\Branch;
use App\Models\Department;
use App\Models\Employee;
use App\Models\Utility;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class AnnouncementController extends Controller
{
    public function index()
    {
        if(\Auth::user()->can('manage announcement'))
        {

            if(Auth::user()->type == 'Employee')
            {
                $current_employee = Employee::where('user_id', '=', \Auth::user()->id)->first();
                $announcements    = Announcement::orderBy('announcements.id', 'desc')->leftjoin('announcement_employees', 'announcements.id', '=', 'announcement_employees.announcement_id')->where('announcement_employees.employee_id', '=', $current_employee->id)->orWhere(
                    function ($q){
                        $q->where('announcements.department_id', '["0"]')->where('announcements.employee_id', '["0"]');
                    }
                )->get();
            }
            else
            {
                $user = \Auth::user();
                $ownerId = $user->type === 'company' ? $user->creatorId() : $user->ownedId();
                $column = ($user->type == 'company') ? 'created_by' : 'owned_by';
                $current_employee = Employee::where('user_id', '=', \Auth::user()->id)->first();
                $announcements    = Announcement::where($column, '=', $ownerId)->get();
            }

            return view('announcement.index', compact('announcements', 'current_employee'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }

    public function create()
    {
        if(\Auth::user()->can('create announcement'))
        {
            $user = \Auth::user();
            $ownerId = $user->type === 'company' ? $user->creatorId() : $user->ownedId();
            $column = ($user->type == 'company') ? 'created_by' : 'owned_by';
            $employees   = Employee::where($column, '=',$ownerId)-> where(function($query) {
    $query->whereNull('termination_date')
          ->orWhere('termination_date', '>', now()->endOfMonth());}) ->get()->pluck('name', 'id');
            $branch      = Branch::where($column, '=',$ownerId)->get();
            $departments = Department::where($column, '=',$ownerId)->get();

            return view('announcement.create', compact('employees', 'branch', 'departments'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }

    public function store(Request $request)
    {


        \DB::beginTransaction();
        try {
        if(\Auth::user()->can('create announcement'))
        {
            $validator = \Validator::make(
                $request->all(), [
                                   'title' => 'required',
                                   'start_date' => 'required',
                                   'end_date' => 'required',
                                   'branch_id' => 'required',
                                   'department_id' => 'required',
                                   'employee_id' => 'required',
                               ]
            );
            if($validator->fails())
            {
                $messages = $validator->getMessageBag();

                return redirect()->back()->with('error', $messages->first());
            }
            $announcement = new Announcement();
            $announcement->title         = $request->title;
            $announcement->start_date    = $request->start_date;
            $announcement->end_date      = $request->end_date;
            $announcement->branch_id     = !empty($request) ? $request->branch_id : 0;
            $announcement->department_id = json_encode($request->department_id);
            $announcement->description   = $request->description;
            $announcement->created_by    = \Auth::user()->creatorId();
            $announcement->owned_by      = \Auth::user()->ownedId();
            
            $announcement->save();
            if($announcement->employee_id){
                $emp = Employee::find($announcement->employee_id);
                $emp->termination_date = $announcement->start_date;
                $emp->save();
            }
            \DB::commit();
            
            $employeeIds = [];
            if (!empty($request->branch_id)) {
                $branchEmployeeIds = Employee::where('branch_id', $request->branch_id)
                    ->pluck('id')
                    ->toArray();
                $employeeIds = array_merge($employeeIds, $branchEmployeeIds);
            }
            if (!empty($request->department_id)) {
                $departmentEmployeeIds = Employee::whereIn('department_id', $request->department_id)
                    ->pluck('id')
                    ->toArray();
                $employeeIds = array_merge($employeeIds, $departmentEmployeeIds);
            }
            $employeeIds = array_unique($employeeIds);
            $userarr = array_merge(
                [
                    \Auth::user()->id, 
                ],
                $employeeIds 
            );

            // Remove empty values
            $userarr = array_filter($userarr, function ($value) {
                return !is_null($value) && $value !== '';
            });
            
            if ($announcement->employee) {
                $userarr[] = $announcement->employee->report_to ?? null;
                $userarr[] = $announcement->employee->id ?? null;
            }
            $userarr = array_filter($userarr);
            
            $dataarr = array_filter([
                "updated_by" => Auth::user()->id,
                "data_id" => $announcement->id,
            ]);
            foreach($userarr as $key => $notifyto){
                Utility::makeNotification($notifyto,'announcement',$dataarr,$announcement->id,' Made Announcement ',\Auth::user()->name);
            }
            if(in_array('0', $request->employee_id))
            {
                $departmentEmployee = Employee::whereIn('department_id', $request->department_id)->get()->pluck('id');
                $departmentEmployee = $departmentEmployee;

            }
            else
            {
                $departmentEmployee = $request->employee_id;
            }
            foreach($departmentEmployee as $employee)
            {
                $announcementEmployee                  = new AnnouncementEmployee();
                $announcementEmployee->announcement_id = $announcement->id;
                $announcementEmployee->employee_id     = $employee;
                $announcementEmployee->created_by      = \Auth::user()->creatorId();
                $announcementEmployee->save();
            }

            //For Notification
            $setting  = Utility::settings(\Auth::user()->creatorId());
            if($request->branch_id == 0)
            {
                $branch = Branch::get()->pluck('name' , 'id')->toArray();
            }
            else
            {
                $branch = Branch::find($request->branch_id);
                $branch = explode(',',$branch->name);
            }

            $announceNotificationArr = [
                'announcement_title' =>  $request->title,
                'branch_name' =>  implode(',',$branch),
                'start_date' =>  $request->start_date,
                'end_date' =>  $request->end_date,
            ];
            //Slack Notification
            if(isset($setting['announcement_notification']) && $setting['announcement_notification'] ==1)
            {
                Utility::send_slack_msg('new_announcement', $announceNotificationArr);
            }
            //Telegram Notification
            if(isset($setting['telegram_announcement_notification']) && $setting['telegram_announcement_notification'] ==1)
            {
                Utility::send_telegram_msg('new_announcement', $announceNotificationArr);
            }

            //webhook
            $module ='New Announcement';
            $webhook=  Utility::webhookSetting($module);
            if($webhook)
            {
                $parameter = json_encode($announcement);
                $status = Utility::WebhookCall($webhook['url'],$parameter,$webhook['method']);
                if($status == true)
                {
                    Utility::makeActivityLog(\Auth::user()->id,'Announcemenet',$announcement->id,'Create Announcemenet',$announcement->title);
                    return redirect()->back()->with('success', __('Announcement successfully created.'));
                }
                else
                {
                    return redirect()->back()->with('error', __('Webhook call failed.'));
                }
            }
            Utility::makeActivityLog(\Auth::user()->id,'Announcemenet',$announcement->id,'Create Announcemenet',$announcement->title);
            // return redirect()->route('announcement.index')->with('success', __('Announcement successfully created.'));
            $html = view('announcement.appendrow', compact('announcement'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "announcement-table",
                'action' => 'add',
                'row_id' => $announcement->id,
            ];
            return response()->json(['success' => true, 'message' => __('Announcement successfully created.'), 'data' => $data]);
        }

        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
            \DB::rollback();
            return redirect()->back()->with('error', $e);
        }
    }

    public function show(Announcement $announcement)
    {
        return redirect()->route('announcement.index');
    }

    public function edit($announcement)
    {
        if(\Auth::user()->can('edit announcement'))
        {
            $announcement = Announcement::find($announcement);
            if($announcement->created_by == Auth::user()->creatorId())
            {
                $user = \Auth::user();
                $ownerId = $user->type === 'company' ? $user->creatorId() : $user->ownedId();
                $column = ($user->type == 'company') ? 'created_by' : 'owned_by';
                $branch      = Branch::where($column, $ownerId)->get()->pluck('name', 'id');
                $departments = Department::where($column, $ownerId)->get()->pluck('name', 'id');

                return view('announcement.edit', compact('announcement', 'branch', 'departments'));
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }

    public function update(Request $request, Announcement $announcement)
    {
        \DB::beginTransaction();
        try {
        if(\Auth::user()->can('edit announcement'))
        {
            if($announcement->created_by == \Auth::user()->creatorId())
            {

                $validator = \Validator::make(
                    $request->all(), [
                                       'title' => 'required',
                                       'start_date' => 'required',
                                       'end_date' => 'required',
                                       'branch_id' => 'required',
                                       'department_id' => 'required',
                                   ]
                );
                if($validator->fails())
                {
                    $messages = $validator->getMessageBag();

                    return redirect()->back()->with('error', $messages->first());
                }

                $announcement->title         = $request->title;
                $announcement->start_date    = $request->start_date;
                $announcement->end_date      = $request->end_date;
                $announcement->branch_id     = $request->branch_id;
                $announcement->department_id = $request->department_id;
                $announcement->description   = $request->description;
                $announcement->save();
                if($announcement->employee_id){
                    $emp = Employee::find($announcement->employee_id);
                    $emp->termination_date = $announcement->start_date;
                    $emp->save();
                }
                \DB::commit();
                // log
                Utility::makeActivityLog(\Auth::user()->id,'Announcemenet',$announcement->id,'Update Announcemenet',$announcement->title);
                $html = view('announcement.appendrow', compact('announcement'))->render();
                $data = [
                    'datarow' => $html,
                    'table_id' => "announcement-table",
                    'action' => 'edit',
                    'row_id' => $announcement->id,
                ];
                return response()->json(['success' => true, 'message' => __('Announcement successfully updated.'), 'data' => $data]);
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
            \DB::rollback();
            return redirect()->back()->with('error', $e);
        }
    }

    public function destroy(Announcement $announcement)
    {
        if(\Auth::user()->can('delete announcement'))
        {
            if($announcement->created_by == \Auth::user()->creatorId())
            {
                // log
                Utility::makeActivityLog(\Auth::user()->id,'Announcemenet',$announcement->id,'Delete Announcemenet',$announcement->title);
                $announcement->delete();

                return redirect()->route('announcement.index')->with('success', __('Announcement successfully deleted.'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }

    public function getdepartment(Request $request)
    {
        $user = \Auth::user();
        $ownerId = $user->type === 'company' ? $user->creatorId() : $user->ownedId();
        $column = ($user->type == 'company') ? 'created_by' : 'owned_by';
        if($request->branch_id == 0)
        {
            $departments = Department::where($column, '=',$ownerId)->get()->pluck('name', 'id')->toArray();
        }
        else
        {
            $departments = Department::where($column, '=',$ownerId)->where('branch_id', $request->branch_id)->get()->pluck('name', 'id')->toArray();
        }

        return response()->json($departments);
    }

//    public function getemployee(Request $request)
//    {
//        if(in_array('0', $request->department_id))
//        {
//            $employees = Employee::where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id')->toArray();
//        }
//        else
//        {
//            $employees = Employee::where('created_by', '=', \Auth::user()->creatorId())->whereIn('department_id', $request->department_id)->get()->pluck('name', 'id')->toArray();
//        }
//
//        return response()->json($employees);
//    }


    public function getemployee(Request $request)
    {
        // dd(department_id);
        $user = \Auth::user();
        $ownerId = $user->type === 'company' ? $user->creatorId() : $user->ownedId();
        $column = ($user->type == 'company') ? 'created_by' : 'owned_by';
        if(!$request->department_id )
        {
            $employees = Employee::where($column, '=',$ownerId)->   where(function($query) {
    $query->whereNull('termination_date')
          ->orWhere('termination_date', '>', now()->endOfMonth());}) ->get()->pluck('name', 'id')->toArray();
        }
        else
        {
            $employees = Employee::where($column, '=',$ownerId)->whereIn('department_id', $request->department_id)->where(function($query) {
    $query->whereNull('termination_date')
          ->orWhere('termination_date', '>', now()->endOfMonth());}) ->get()->pluck('name', 'id')->toArray();
        }

        return response()->json($employees);
    }


}
