...
class VenderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create vender'))
        {
            ...
            \DB::commit();
            $html = view('vender.appendrow', compact('vender'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "vender-table",
                'action' => 'add',
                'row_id' => $vender->id,
            ];
            return response()->json(['success' => true, 'message' => __('Vendor successfully created.'), 'data' => $data]);
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function show($ids)
    {
        try {
            ...
            return response()->json(['error' => __('Vendor Not Found.')], 401);
        }
    ...
    public function update(Request $request, Vender $vender)
    {
        ...
        if(\Auth::user()->can('edit vender'))
        {
            ...
            return response()->json(['success' => true, 'message' => __('Vendor successfully updated.'), 'data' => $data]);
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function destroy(Vender $vender)
    {
        if(\Auth::user()->can('delete vender'))
        {
            if($vender->created_by == \Auth::user()->creatorId())
            {
                ...
                return response()->json(['success' => true, 'message' => __('Vendor successfully deleted.')]);
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
