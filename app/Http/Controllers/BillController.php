...
class BillController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            if (\Auth::user()->can('create bill')) {
                ...
                if ($webhook) {
                    ...
                    if ($status == true) {
                        ...
                        \DB::commit();
                        return response()->json(['success' => true, 'message' => __('Bill successfully created.')]);
                    } else {
                ...
                Utility::makeActivityLog(\Auth::user()->id, 'Bill', $bill->id, 'Create Bill', $bill->type);
                \DB::commit();
                // $html = view('bill.appendrow', compact('bill'))->render();
                // $data = [
                //     'datarow' => $html,
                //     'table_id' => "bill-table",
                //     'action' => 'add',
                //     'row_id' => $bill->id,
                // ];
                // return response()->json(['success' => true, 'message' => __('Bill successfully created.'), 'data' => $data]);
            ...
            \DB::rollback();
            return response()->back()->with('error', $e);
        }
    }
    ...
    public function show($ids)
    {
        ...
        if (\Auth::user()->can('show bill')) {
            try {
                ...
                return response()->json(['error' => __('Bill Not Found.')], 401);
            }
        ...
        try {
            if (\Auth::user()->can('edit bill')) {
                ...
                if ($bill->created_by == \Auth::user()->creatorId()) {
                    ...
                    // Update voucher if bill has already been sent (has a voucher_id)
                    if (!empty($bill->voucher_id)) {
                        // Delete old journal entries and items
                        $oldJournal = \App\Models\JournalEntry::find($bill->voucher_id);
                        if ($oldJournal) {
                            // Delete journal items
                            \App\Models\JournalItem::where('journal', $oldJournal->id)->delete();
                            // Delete related transaction lines
                            TransactionLines::where('reference', 'Bill Journal')
                    ...
                    //     'row_id' => $bill->id,
                    // ];
                    // return response()->json(['success' => true, 'message' => __('Bill successfully updated.'), 'data' => $data]);
                    return redirect()->route('bill.index')->with('success', __('Bill successfully updated.'));
                } else {
                    return response()->json(['error' => __('Permission denied.')], 401);
                }
        ...
        if (\Auth::user()->can('delete bill')) {
            if ($bill->created_by == \Auth::user()->creatorId()) {
                ...
                foreach ($billpayments as $key => $value) {
                    ...
                    // Delete payment voucher if exists (use payment's voucher_id, not bill's)
                    if (!empty($value->voucher_id)) {
                        $journal = \App\Models\JournalEntry::find($value->voucher_id);
                        if ($journal) {
                            // Delete journal items
                            \App\Models\JournalItem::where('journal', $journal->id)->delete();
                            // Delete related transaction lines for Bill Payment
                            TransactionLines::where('reference', 'Bill Payment')
                ...
                // Delete voucher (journal entry and items) if exists
                if (!empty($bill->voucher_id)) {
                    $journal = \App\Models\JournalEntry::find($bill->voucher_id);
                    if ($journal) {
                        // Delete journal items
                        \App\Models\JournalItem::where('journal', $journal->id)->delete();
                        // Delete related transaction lines for Bill Journal
                        TransactionLines::where('reference', 'Bill Journal')
                            ->where('reference_id', $journal->id)
                            ->delete();
                        // Delete the journal entry
    ...
    public function productDestroy(Request $request)
    {
        ...
        if (\Auth::user()->can('delete bill product')) {
            ...
            // Update voucher if bill has been sent (has a voucher_id)
            if (!empty($bill->voucher_id)) {
                // Delete old journal entries and items
                $oldJournal = \App\Models\JournalEntry::find($bill->voucher_id);
                if ($oldJournal) {
                ...
                // Recreate the voucher with updated products
                $this->createBillJournalVoucher($bill);
            }
            ...
            if (request()->ajax()) {
                return response()->json(['success' => true, 'message' => __('Bill product successfully deleted.')]);
            }
    ...
    public function payment($bill_id)
    {
        if (\Auth::user()->can('create payment bill')) {
            ...
            return view('bill.payment', compact('venders', 'categories', 'accounts', 'bill', 'types'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function createPayment(Request $request, $bill_id)
    {
        if (\Auth::user()->can('create payment bill')) {
            ...
            $bill = Bill::with(['payments.bankAccount'])->find($bill_id);
            ...
            $validator = \Validator::make(
                ...
                [
                    ...
                    'cheque_number'    => 'nullable|required_if:type,cheque|max:100',
                    'title_of_account' => 'nullable|required_if:type,cheque|max:191', // <— NEW
                ],
                // [
            ...
            $billPayment->type = strtolower(trim($request->type));
            $billPayment->cheque_number = $billPayment->type === 'cheque' ? ($request->cheque_number ?? null) : null;
            ...
            $billPayment->save();
            // ===== Checklist writes =====
            // if ($billPayment->type === 'cheque') {
            //     $statusForType = 'Pending';

            //     $billPayment->checkList()->create([
            //         'invoice_id'         => null,
            //         'invoice_payment_id' => null,

            //         'bill_id'       => $bill_id,
            //         'bill_payment_id' => $billPayment->id,               // optional but nice to set if your model supports it
            //         'account_id'    => $request->account_id,
            //         'account_title' => $request->title_of_account,      // <— NEW: save Title of Account
            //         'cheque_number' => $billPayment->cheque_number,
            //         'amount'        => $request->amount,
            //         'date'          => $request->date,
            //         'type'          => $billPayment->type,              // 'cheque'
            //         'status'        => $statusForType,
            //         'created_by'    => \Auth::id(),
            //         'owned_by'      => \Auth::user()->ownedId(),
            //     ]);
            // } else {
            //     // (Optional) log non-cheque methods as well
            //     $billPayment->checkList()->create([
            //         'invoice_id'         => null,
            //         'invoice_payment_id' => null,

            //         'bill_id'       => $bill_id,
            //         'bill_payment_id' => $billPayment->id,               // optional
            //         'account_id'    => $request->account_id,
            //         'account_title' => null,                            // not applicable
            //         'cheque_number' => null,
            //         'amount'        => $request->amount,
            //         'date'          => $request->date,
            //         'type'          => $billPayment->type,              // 'cash' | 'direct_deposit'
            //         'status'        => 'Recorded',
            //         'created_by'    => \Auth::id(),
            //         'owned_by'      => \Auth::user()->ownedId(),
            //     ]);
            ...
            // Create payment voucher (CPV for cash, BPV for bank)
            $bankAccount = BankAccount::find($request->account_id);
            
            // Determine vendor/customer/employee based on bill user_type
            $vendor_id = null;
            $customer_id = null;
            $employee_id = null;
            if ($bill->user_type == 'vendor') {
                ...
                $customer_id = $bill->vender_id;
            } elseif ($bill->user_type == 'employee') {
                $employee_id = $bill->vender_id;
            }
            ...
            if($bill->user_type == 'customer'){
                $customer_id = $bill->vender_id;
            }
            if($bill->user_type == 'employee'){
                $employee_id = $bill->vender_id;
            }
            ...
            $voucherData = [
                ...
                'description' => $request->description ?? 'Bill Payment',
                'created_at' => date('Y-m-d H:i:s', strtotime($payment->date.' '.date('H:i:s'))),
                'category' => 'Bill',
                'prod_id' => $billPayment->id,
            ...
            // Use CPV for cash accounts, BPV for bank accounts
            if (preg_match('/\bcash\b/i', $bankAccount->bank_name) || preg_match('/\bcash\b/i', $bankAccount->holder_name)) {
                $voucherId = Utility::cpv_entry($voucherData); // Cash Payment Voucher
            } else {
                $voucherId = Utility::bpv_entry($voucherData); // Bank Payment Voucher
            }
            
            // Save voucher_id to bill payment
            $billPaymentRecord = BillPayment::find($billPayment->id);
            $billPaymentRecord->voucher_id = $voucherId;
    ...
    public function paymentDestroy(Request $request, $bill_id, $payment_id)
    {
        ...
        if (\Auth::user()->can('delete payment bill')) {
            ...
            // Delete voucher (journal entry and items) if payment has one
            if (!empty($payment->voucher_id)) {
                ...
                if ($journal) {
                    ...
                    \App\Models\JournalItem::where('journal', $journal->id)->delete();
                    // Delete related transaction lines
                    $tra = TransactionLines::where('reference', 'like', '%Payment%')
                        ->where('reference_id', $journal->id)
                        ->delete();
                    // Delete the journal entry
                    $journal->delete();
     ...
     private function createBillJournalVoucher(Bill $bill)
    {
        ...
        $data = [
            ...
            'employee_id' => $employee_id,
            'date' => $bill->bill_date,
            'created_at' => date('Y-m-d H:i:s', strtotime($bill->bill_date. ' ' .date('H:i:s'))),
