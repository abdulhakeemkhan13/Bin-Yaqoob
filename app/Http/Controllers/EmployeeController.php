...
class EmployeeController extends Controller
{
            ...
            if (\Auth::user()->can('create employee')) {
                $validator = \Validator::make(
                    ...
                    [
                        ...
                        'deployed_at' => 'nullable|string|max:255',
                        'deployed_date' => 'nullable|date',
                    ]
                );

                $designation = Designation::find($request->designation_id);
                $isGuard = $designation && str_contains(strtolower($designation->name), 'guard');

                // Conditionally require when designation is Guard
                $validator->sometimes('deployed_at', 'required|string|max:255', fn() => $isGuard);
                $validator->sometimes('deployed_date', 'required|date', fn() => $isGuard);
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit employee')) {
            ...
            $guardDesignation = Designation::where('name', 'like', '%guard%')
                ->where(function ($q) {
                    if (\Auth::user()->type == 'company') {
            ...
            return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents', 'departmentData', 'employee_report', 'customFields', 'guardDesignation'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Determine if selected designation is Guard (tolerate "gaurd")
        $designation = null;
        ...
        // Conditionally require the new fields for Guard
        $validator->sometimes('deployed_at', 'required|string|max:255', fn() => $isGuard);
        $validator->sometimes('deployed_date', 'required|date', fn() => $isGuard);
        ...
        // Handle document uploads (unchanged)
        if ($request->document) {
            foreach ($request->document as $key => $document) {
                if (!empty($document)) {
                    $filenameWithExt = $request->file('document')[$key]->getClientOriginalName();
        ...
        // Prepare input for mass assignment
        $input = $request->all();

        // If not Guard, clear these so stale values don't persist
        if (!$isGuard) {
            $input['deployed_at'] = null;
            $input['deployed_date'] = null;
        }
        ...
        // Sync the linked user’s basic info
    ...
    public function incrementForm()
    {
        // Minimal fields for the dropdown
        $employees = Employee::orderBy('name')->get(['id', 'name', 'salary']);

        // Last increment per employee (if exists)
        $lastIncrements = EmployeeIncrement::select('employee_id', 'increment_date')
            ->latest('increment_date')
            ->get()
            ->keyBy('employee_id');

        // Build a ready-to-use payload for the modal JS (no AJAX needed later)
        $empPayload = $employees->mapWithKeys(function ($e) use ($lastIncrements) {
            $last = $lastIncrements->get($e->id);
            return [
                $e->id => [
                    'id'                => $e->id,
                    'name'              => $e->name,
                    'previous_salary'   => (float) ($e->salary ?? 0),
                    'last_increment_at' => $last && $last->increment_date
                        ? \Illuminate\Support\Carbon::parse($last->increment_date)->format('Y-m-d')
                        : null,
                ],
            ];
        });

        return view('employee.increment_modal', [
            'employees'   => $employees,
            'empPayload'  => $empPayload, // pass precomputed data for JS
        ]);
    }

    public function incrementInfo($id)
    {
        // Get ONE employee, not a collection
        $emp = Employee::select('id', 'name', 'salary')->findOrFail($id);

        // Optional: last increment date from history table
        $last = EmployeeIncrement::where('employee_id', $emp->id)
            ->latest('increment_date')
            ->first();
        ...
        return response()->json([
            'id'                => $emp->id,
            'name'              => $emp->name,
            'previous_salary'   => (float) ($emp->salary ?? 0),
            'last_increment_at' => optional($last)->increment_date
                ? $last->increment_date->format('Y-m-d')
                : null,
        ]);
    }

    public function employeeIncrement(Request $request)
    {
        ...
        // Save increment history (new row each time)
        EmployeeIncrement::create([
            ...
            'new_salary'      => $emp->salary + $data['increment_amount'],
            'created_by'      => auth()->id(),
        ]);

        // Apply to current salary
        $emp->salary = $emp->salary + $data['increment_amount'];
        $emp->save();

        return redirect()->back()->with('success', __('Employee salary increment successfully'));
    }

    public function incrementHistory($id)
    {
        $emp = Employee::findOrFail($id);

        $rows = EmployeeIncrement::where('employee_id', $emp->id)
        ...
        // $rows->load('creator'); // if you have a relation

        return view('employee.increment_history_modal', compact('emp', 'rows'));
    }
}
