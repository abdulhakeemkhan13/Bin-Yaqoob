...
class LeaveController extends Controller
{
    ...
    public function create()
    {
        ...
        if (in_array($user->type, ['company', 'HR','branch'])) {
            $ownerId = $user->type == 'company' ? $creatorId : $ownedId;
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create leave')) {
            ...
            if ($leave_type->days >= $total_leave_days) {
                ...
                $leave->save();
                $leave->load('employees');
                if($leave->employee_id){
                    $emp = Employee::find($leave->employee_id);
                    $emp->termination_date = $leave->end_date;
                    $emp->save();
                }
                // For Notification
                //send to users 
                ...
                // dd($userarr);
                \DB::commit();
                $html = view('leave.appendrow', compact('leave'))->render();
                $data = [
                    'datarow' => $html,
                ...
                return response()->json(['success' => true, 'message' => __('Leave successfully created.'), 'data' => $data]);
            } else {
                return response()->json(['error' => __('Leave type ' . $leave_type->name . ' is provide maximum ' . $leave_type->days . "  days please make sure your selected days is under " . $leave_type->days . ' days.')], 401);
            }
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
        ...
        if (\Auth::user()->can('edit leave')) {
            if ($leave->created_by == Auth::user()->creatorId()) {
                ...
                if ($leave_type->days >= $total_leave_days) {
                    ...
                    $leave->remark = $request->remark;
                    $leave->save();
$leave->loadMissing('employees'); // ← make sure relation is loaded
...
// Safe notification users
$userarr = collect([
    \Auth::user()->id,
    $leave->employees?->report_to,
    User::where('type', 'hr')->value('id'), // ← faster & safer
])->filter()->unique()->values()->toArray();

// Safe employee name for notification
...
// Safe activity log (this will NEVER crash)
Utility::makeActivityLog(
    ...
    $employeeName
                    ...
                    return response()->json(['success'=>true,'message' => __('Leave successfully created.'),"data"=>$data,200]);
    ...
    public function destroy(Leave $leave)
{
    if (\Auth::user()->can('delete leave') && $leave->created_by == \Auth::user()->creatorId()) {
        $leave->loadMissing('employees');  // Load here

        $empName = $leave->employees?->name ?? 'Unknown Employee';
    ...
    public function changeaction(Request $request)
    {
        \DB::beginTransaction();
        try {
        $leave = Leave::with('employees')->find($request->leave_id);
        ...
        $leave->save();
$leave->loadMissing('employees'); // ← VERY IMPORTANT

$employeeName = $leave->employees?->name ?? 'Unknown Employee';
$reportToId   = $leave->employees?->report_to;

$userarr = collect([
    \Auth::user()->id,
    $reportToId,
    User::where('type', 'hr')->value('id'),
])->filter()->unique()->values()->toArray();
...
foreach ($userarr as $notifyto) {
    Utility::makeNotification(
        ...
        $leave->id,
        'Applied action on Leave'
    );
}

// Later in the same function, replace both activity logs:
Utility::makeActivityLog(\Auth::user()->id, 'Leave', $leave->id, 'Leave Status Changed', $employeeName);
        $employee = Employee::find($leave->employee_id);
    ...
    public function report_to_create()
{
    ...
    // Get IDs of employees who report to current logged-in user (manager)
    $reportToMeIds = Employee::where('report_to', @\Auth::user()->employee->id)
        ->pluck('id')
        ->toArray();

    // Apply termination filter (THIS WAS THE BUG!)
    $employees = $employees->where(function ($q) {
        $q->whereNull('termination_date')
          ->orWhere('termination_date', '>', now()); // Correct: active today or future
    });

    // If current user is a manager with subordinates → show only them
    if (!empty($reportToMeIds)) {
        $employees = $employees->whereIn('id', $reportToMeIds); // use 'id', not 'employee_id'
    }
    // If no subordinates → dropdown will be empty (or you can allow self, up to you)
