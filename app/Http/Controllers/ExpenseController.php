...
class ExpenseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create bill')) {
            ...
            // Build items array for voucher
            $newitems = [];
            $bill_products_for_voucher = BillProduct::where('bill_id', $expense->id)->get();
            foreach ($bill_products_for_voucher as $product) {
            ...
            // Get bank account for voucher type determination
            $bank = BankAccount::find($request->account_id);

            // Determine vendor_id, customer_id, employee_id based on user_type
            $vendor_id = null;
            $customer_id = null;
            $employee_id = null;
            if ($expense->user_type == 'vendor') {
                ...
                $customer_id = $expense->vender_id;
            } elseif ($expense->user_type == 'employee') {
                $employee_id = $expense->vender_id;
            }
            ...
            $data['owned_by'] = $expense->owned_by ?? \Auth::user()->ownedId();
            ...
            if (preg_match('/\bcash\b/i', $bank->bank_name) || preg_match('/\bcash\b/i', $bank->holder_name)) {
                ...
                $dataret = Utility::bpv_entry($data); // Bank Payment Voucher (BPV)
            }
    ...
    public function update(Request $request, $id)
    {
        if (\Auth::user()->can('edit bill')) {
            ...
            if ($expense->created_by == \Auth::user()->creatorId()) {
                ...
                // Update voucher: delete old voucher and recreate
                if (!empty($expense->voucher_id)) {
                    // Delete old voucher and its related records
                    $journal = \App\Models\JournalEntry::find($expense->voucher_id);
                    if ($journal) {
                        \App\Models\JournalItem::where('journal', $journal->id)->delete();
                        TransactionLines::where('reference_id', $journal->id)->where('reference', 'Expense Journal')->delete();
                        $journal->delete();
                    }
                }

                // Recreate voucher with updated data
                $this->createExpenseJournalVoucher($expense, $request->account_id, $request->totalAmount);
    ...
    public function productDestroy(Request $request)
    {
        ...
        if (\Auth::user()->can('delete bill product')) {
            ...
            // Update voucher if expense has one (delete old and recreate)
            if (!empty($expense->voucher_id)) {
                // Delete old voucher
                $journal = \App\Models\JournalEntry::find($expense->voucher_id);
                if ($journal) {
                ...
                // Get the payment to get account_id and amount
                $expensePayment = BillPayment::where('bill_id', $expense->id)->first();
                if ($expensePayment) {
                    // Recreate voucher with updated products
                    $this->createExpenseJournalVoucher($expense, $expensePayment->account_id, $expense->getTotal());
                }
            }

            // Return JSON for AJAX requests
            if ($request->ajax()) {
                return response()->json(['success' => true, 'message' => __('Expense product successfully deleted.')]);
            }
            ...
            if ($expense->created_by == \Auth::user()->creatorId()) {
                ...
                foreach ($expensepayments as $key => $value) {
                    ...
                    if (!empty($value->voucher_id)) {
                        $paymentJournal = \App\Models\JournalEntry::find($value->voucher_id);
                        if ($paymentJournal) {
                            \App\Models\JournalItem::where('journal', $paymentJournal->id)->delete();
                            TransactionLines::where('reference_id', $paymentJournal->id)->delete();
                    ...
                    $expensepayment = BillPayment::find($value->id);
                    if ($expensepayment) {
                ...
                // TransactionLines are already deleted with voucher above
    ...
    /**
     * Create expense journal voucher (CPV/BPV)
     */
    private function createExpenseJournalVoucher($expense, $account_id, $amount)
    {
        // Build items array for voucher
        $newitems = [];
        $bill_products = BillProduct::where('bill_id', $expense->id)->get();
        ...
        // Get bank account for voucher type determination
        $bank = BankAccount::find($account_id);
        ...
        $data = [
            ...
            'no' => $expense->bill_id,
            'date' => $expense->bill_date,
            'created_at' => date('Y-m-d', strtotime($expense->bill_date)) . ' ' . date('h:i:s'),
            'reference' => $expense->ref_number ?? '',
            'category' => 'Expanse',
            'owned_by' => $expense->owned_by ?? \Auth::user()->ownedId(),
            'created_by' => $expense->created_by,
            'prod_id' => $expensePayment ? $expensePayment->id : 0,
            'amount' => $amount,
            'account_id' => $bank->chart_account_id ?? null,
            'items' => $newitems,
            'vendor_id' => $vendor_id,
        ...
        if ($bank && (preg_match('/\bcash\b/i', $bank->bank_name) || preg_match('/\bcash\b/i', $bank->holder_name))) {
        ...
        // Update payment with voucher_id
        if ($expensePayment) {
