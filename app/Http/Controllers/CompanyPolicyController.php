    ...
    public function index()
    {
        if(\Auth::user()->can('manage company policy'))
        {
            ...
            if ($user->type == 'company') {
                ...
                $companyPolicy = CompanyPolicy::where(function ($query) use ($user) {
...
//            }
            ...
            if (!empty($request->branch)) {
                $branchEmployeeIds = Employee::where('branch_id', $request->branch)
                    ->pluck('id')
                    ->toArray();
            ...
            foreach ($userarr as $key => $notifyto) {
                Utility::makeNotification($notifyto, 'policy', $dataarr, $policy->id, 'Introduce a new ' . $policy->title. ' Policy', \Auth::user()->name);
            }
            // dd($request->all());
            //For Notification
            ...
            Utility::makeActivityLog(\Auth::user()->id,'Comapny Policy',$policy->id,'Create Comapny Policy',$policy->title);
            // return redirect()->route('company-policy.index')->with('success', __('Company policy successfully created.'));
            $html = view('companyPolicy.appendrow', ['companyPolicy' => [$policy]])->render();
            $data = [
                'datarow' => $html,
                'table_id' => "company-policy-table",
                'action' => 'add',
                'row_id' => $policy->id,
            ];
            return response()->json(['success' => true, 'message' => __('Company policy successfully created.'), 'data' => $data]);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
...
//            }
            ...
            $companyPolicy->save();
            DB::commit();
            Utility::makeActivityLog(\Auth::user()->id,'Comapny Policy',$companyPolicy->id,'Update Comapny Policy',$companyPolicy->title);
            // return redirect()->route('company-policy.index')->with('success', __('Company policy successfully updated.'));
            $html = view('companyPolicy.appendrow', ['companyPolicy' => [$companyPolicy]])->render();
            $data = [
                'datarow' => $html,
                'table_id' => "company-policy-table",
                'action' => 'edit',
                'row_id' => $companyPolicy->id,
            ];
            return response()->json(['success' => true, 'message' => __('Company policy successfully updated.'), 'data' => $data]);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function destroy(CompanyPolicy $companyPolicy)
    {
        ...
        if(\Auth::user()->can('delete document'))
        {
            ...
            {
                Utility::makeActivityLog(\Auth::user()->id,'Comapny Policy',$companyPolicy->id,'Delete Comapny Policy',$companyPolicy->title);
