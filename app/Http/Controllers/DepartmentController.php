...
class DepartmentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create department')) {
            ...
            $html = view('department.appendrow', compact('department'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "department-table",
                'action' => 'add',
                'row_id' => $department->id,
            ];
            return response()->json(['success' => true, 'message' => __('Department  successfully created.'), 'data' => $data]);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, Department $department)
    {
        ...
        if (\Auth::user()->can('edit department')) {
            if ($department->created_by == \Auth::user()->creatorId()) {
                ...
                return response()->json(['success' => true, 'message' => __('Department successfully updated.'), 'data' => $data]);
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
