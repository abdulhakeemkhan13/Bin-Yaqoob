...
class PaySlipController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->type == 'company') {
            $payslip_employee = Employee::where('created_by', \Auth::user()->creatorId())
                ...
                ->count();
            $resignation = Resignation::where('created_by', \Auth::user()->creatorId())
                ->where('resignation_date', '<=', date('Y-m-d'))
                ->pluck('employee_id')
                ->toArray();
            $termination = Termination::where('created_by', \Auth::user()->creatorId())
                ->where('termination_date', '<=', date('Y-m-d'))
                ->pluck('employee_id')
                ->toArray();
        } else {
            $payslip_employee = Employee::where('owned_by', \Auth::user()->ownedId())
                ->where('company_doj', '<=', date($year . '-' . $month . '-t'))
                ->count();
            $resignation = Resignation::where('owned_by', \Auth::user()->ownedId())
                ->where('resignation_date', '<=', date('Y-m-d'))
                ->pluck('employee_id')
                ->toArray();
            $termination = Termination::where('owned_by', \Auth::user()->ownedId())
                ->where('termination_date', '<=', date('Y-m-d'))
                ->pluck('employee_id')
                ->toArray();
        }
        ...
        if ($payslip_employee > count($validatePaysilp)) {
            if (\Auth::user()->type == 'company') {
                $employees = Employee::where('created_by', \Auth::user()->creatorId())
                    ...
                    ->get();
                $employeesSalary = Employee::where('created_by', \Auth::user()->creatorId())
                    ->where('salary', '<=', 0)
                    ->first();
            } else {
                ...
                $employeesSalary = Employee::where('owned_by', \Auth::user()->ownedId())
                    ->where('salary', '<=', 0)
                    ->first();
            }
            ...
            foreach ($employees as $employee) {
                ...
                // PRORATED basic + month-aware net
                $payslipEmployee->basic_salary         = $employee->proratedBasic($formate_month_year);
                ...
                $payslipEmployee->owned_by             = \Auth::user()->ownedId();
                $payslipEmployee->save();

                // notifications/webhooks unchanged...
                $setting  = Utility::settings(\Auth::user()->creatorId());
                $payslipNotificationArr = ['year' =>  $formate_month_year];
                ...
                Utility::makeActivityLog(\Auth::user()->id, 'payslip', $payslipEmployee->id, 'Create payslip', $employee->name);
            }

            return redirect()->route('payslip.index')->with('success', __('Payslip successfully created for remaining employees.'));
        } else {
            return redirect()->route('payslip.index')->with('error', __('Payslip already created for all employees.'));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        Utility::makeActivityLog(\Auth::user()->id, 'payslip', $payslip->id, 'Delete payslip', 'payslip_delete');
        $payslip->delete();
    ...
    //     // $workMeta = Employee::workingDays($payslip->employee_id, $payslip->salary_month);

    //     // decode snapshot saved on creation/update (fallback to live compute)
    //     $leaves = $payslip->leaves
    //         ? json_decode($payslip->leaves, true)
    //         : (Employee::leave($payslip->employee_id, $payslip->salary_month) ?: [
    //             'total_leave' => 0,
    //             'paid' => 0,
    //             'unpaid' => 0,
    //             'late' => 0,
    ...
    //     return view('payslip.show', compact('payslip', 'leaves'));
    // }
    ...
    public function search_json(Request $request)
    {
        ...
        if (\Auth::user()->type == 'company') {
            ...
            $validatePaysilp = PaySlip::where('salary_month', '=', $formate_month_year)->where('owned_by', \Auth::user()->ownedId())->get()->toarray();
        }
        $data = [];
        if (empty($validatePaysilp)) {
            ...
            $payslip_employee_query = PaySlip::select(
                [
                    'employees.id',
            ...
            if (\Auth::user()->type == 'company') {
                $paylip_employee = $payslip_employee_query->where('employees.created_by', \Auth::user()->creatorId())->get();
            } else {
                $paylip_employee = $payslip_employee_query->where('employees.owned_by', \Auth::user()->ownedId())->get();
            }
    ...
    public function paysalary($id, $date)
    {
        if (\Auth::user()->type == 'company') {
            ...
            $employeePayslip = PaySlip::where('employee_id', '=', $id)->where('owned_by', \Auth::user()->ownedId())->where('salary_month', '=', $date)->first();
        }
        ...
        if (!empty($employeePayslip)) {
            ...
            Utility::makeActivityLog(\Auth::user()->id, 'Payslip Payment', $employeePayslip->id, 'Create Payslip Payment', $account->name);
    ...
    public function bulk_pay_create($date)
    {
        if (\Auth::user()->type == 'company') {
            ...
            $Employees = PaySlip::where('salary_month', $date)->where('owned_by', \Auth::user()->ownedId())->get();
            $unpaidEmployees = PaySlip::where('salary_month', $date)->where('owned_by', \Auth::user()->ownedId())->where('status', '=', 0)->get();
        }
        //log will be here
        Utility::makeActivityLog(\Auth::user()->id, 'Bulk Payment Creation', $date, 'Create Bulk Payment Creation', 'Bulk Payment Creation');
    ...
    public function pdf($id, $month)
    {
        if (\Auth::user()->type == 'company') {
            ...
            $payslip = PaySlip::where('employee_id', $id)->where('salary_month', $month)->where('owned_by', \Auth::user()->ownedId())->first();
        }
        $employee = Employee::find($payslip->employee_id);
        $payslipDetail = Utility::employeePayslipDetail($id, $month);

        // NEW: last increment on/before the payslip month
        $monthEnd = Carbon::parse($month . '-01')->endOfMonth()->toDateString();
        $lastIncrementDate = EmployeeIncrement::where('employee_id', $id)
            ->whereDate('increment_date', '<=', $monthEnd)
            ->max('increment_date'); // returns 'Y-m-d' or null

        // $workMeta = Employee::workingDays($payslip->employee_id, $payslip->salary_month);
        $leaves = $payslip->leaves ? json_decode($payslip->leaves, true) : Employee::leave($payslip->employee_id, $payslip->salary_month);
        // dd($leaves, $payslip->leaves, $payslipDetail, $payslip);
        return view('payslip.pdf', compact('payslip', 'employee', 'payslipDetail', 'lastIncrementDate', 'leaves'));
    }
    ...
    public function send($id, $month)
    {
        ...
        if ($setings['payslip_sent'] == 1) {
            ...
            // log will be here 
            Utility::makeActivityLog(\Auth::user()->id, 'Payslip Sent', $payslip->id, $payslip->name, 'Payslip Sent');
...
public function payslipPdf($id)
{
    ...
    if (\Auth::user()->type == 'company') {
        ...
        $payslip = PaySlip::where('id', $payslipId)->where('owned_by', \Auth::user()->ownedId())->first();
    }
    ...
    // NEW: last increment on/before the payslip month
    $monthEnd = Carbon::parse($payslip->salary_month . '-01')->endOfMonth()->toDateString();
    $lastIncrementDate = EmployeeIncrement::where('employee_id', $payslip->employee_id)
        ->whereDate('increment_date', '<=', $monthEnd)
        ->max('increment_date');

    return view('payslip.payslipPdf', compact('payslip', 'employee', 'payslipDetail', 'lastIncrementDate'));
}
    ...
    {
        // 1) Update row amounts for the related models (unchanged)
        ...
        if (isset($request->salarytax) && !empty($request->salarytax)) {
            $salarytaxs   = $request->salarytax;
            $salarytaxIds = $request->salarytax_id;
            foreach ($salarytaxs as $k => $tax) {
                $salarytaxData = SalaryTax::find($salarytaxIds[$k]);
                if ($salarytaxData) {
                    $salarytaxData->amount = $tax;
                    $salarytaxData->save();
                }
            }
        }

        if (isset($request->eobi) && !empty($request->eobi)) {
            $eobis   = $request->eobi;
            $eobiIds = $request->eobi_id;
            foreach ($eobis as $k => $amount) {
                $eobi = Eobi::find($eobiIds[$k]);
                if ($eobi) {
                    $eobi->amount = $amount;
                    $eobi->save();
                }
        ...
        // 2) Refresh the payslip JSON blobs and recompute with month-aware base
        ...
        $payslipEmployee->tax                  = Employee::tax($payslipEmployee->employee_id);
        $payslipEmployee->eobi                 = Employee::eobi($payslipEmployee->employee_id);

        $emp   = Employee::find($payslipEmployee->employee_id);
        $month = $payslipEmployee->salary_month; // "YYYY-MM"

        // Recompute PRORATED basic + net for that month
        $payslipEmployee->basic_salary = $emp->proratedBasic($month);
        $payslipEmployee->net_payble   = $emp->get_net_salary($month);

        $payslipEmployee->save();

        Utility::makeActivityLog(\Auth::user()->id, 'Employee Payroll Updated', $payslipEmployee->id, 'Employee Payroll Updated', 'Employee Payroll Updated');
