...
class TerminationTypeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create termination type')) {
            ...
            $html = view('terminationtype.appendrow', compact('terminationtype'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "terminationtype-table",
                'action' => 'add',
                'row_id' => $terminationtype->id,
            ];
            return response()->json(['success' => true, 'message' => __('Termination Type successfully created.'), 'data' => $data]);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (\Auth::user()->can('edit termination type')) {
            ...
            if ($terminationtype->created_by == \Auth::user()->creatorId()) {
                ...
                return response()->json(['success' => true, 'message' => __('Termination Type successfully updated.'), 'data' => $data]);
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
