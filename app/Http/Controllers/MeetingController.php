...
class MeetingController extends Controller
{
        ...
        if(\Auth::user()->can('manage meeting'))
        {
            ...
            else
            {
                if (\Auth::user()->type == 'company') {
                    ...
                    $meetings = Meeting::where('owned_by', '=', \Auth::user()->ownedId())->get();
                }
            }
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create meeting'))
        {
            ...
            if($webhook)
            {
                ...
                if($status == true)
                {
                    Utility::makeActivityLog(\Auth::user()->id,'Meeting',$meeting->id,'Create Meeting',$meeting->title);
            ...
            Utility::makeActivityLog(\Auth::user()->id,'Meeting',$meeting->id,'Create Meeting',$meeting->title);
            $html = view('meeting.appendrow', compact('meeting'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "meeting-table",
                'action' => 'add',
                'row_id' => $meeting->id,
            ];
            return response()->json(['success' => true, 'message' => __('Meeting  successfully created.'), 'data' => $data]);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, Meeting $meeting)
    {
        ...
        if(\Auth::user()->can('edit meeting'))
        {
            ...
            if($meeting->created_by == \Auth::user()->creatorId())
            {
                ...
                Utility::makeActivityLog(\Auth::user()->id,'Meeting',$meeting->id,'Update Meeting',$meeting->title);
                $html = view('meeting.appendrow', compact('meeting'))->render();
                $data = [
                    'datarow' => $html,
                    'table_id' => "meeting-table",
                ...
                return response()->json(['success' => true, 'message' => __('Meeting successfully updated.'), 'data' => $data]);
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function destroy(Meeting $meeting)
    {
        if(\Auth::user()->can('delete meeting'))
        {
            ...
            {
                //log will be here
                Utility::makeActivityLog(\Auth::user()->id,'Meeting',$meeting->id,'Delete Meeting',$meeting->title);
                $meeting->delete();
