...
class CreditNoteController extends Controller
{
    ...
    public function store(Request $request, $invoice_id)
    {
        ...
        if(\Auth::user()->can('create credit note'))
        {
            ...
            {
                return response()->json(['error' => 'Maximum ' . \Auth::user()->priceFormat($invoiceDue->getDue()) . ' credit limit of this invoice.'], 401);
            }
            ...
            \DB::commit();
            $html = view('creditNote.appendrow', ['credit' => $credit])->render();
            $data = [
                'datarow' => $html,
            ...
            return response()->json(['success' => true, 'message' => __('Credit Note successfully created.'), 'data' => $data]);
            // return redirect()->route('credit.note', $invoice_id)->with('success', __('Credit Note successfully created.'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function update(Request $request, $invoice_id, $creditNote_id)
    {
        ...
        if(\Auth::user()->can('edit credit note'))
        {
            ...
            $html = view('creditNote.appendrow', compact('credit'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "credit-table",
                'action' => 'edit',
            ...
            return response()->json(['success' => true, 'message' => __('Credit Note successfully updated.'), 'data' => $data]);
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function destroy($invoice_id, $creditNote_id)
    {
        if(\Auth::user()->can('delete credit note'))
        {
            ...
            return response()->json(['success' => true, 'message' => __('Credit Note successfully deleted.')]);
    ...
    public function customStore(Request $request)
    {
        if(\Auth::user()->can('create credit note'))
        {
            ...
            if(!$invoiceDue) {
                return response()->json(['error' => __('Invoice not found.')], 401);
            }
            ...
            return response()->json(['success' => true, 'message' => __('Credit Note successfully created.'), 'data' => $data]);
            // return response()->json(['success' => true, 'message' => __('Credit Note successfully created.')]);
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
