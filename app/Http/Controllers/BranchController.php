...
class BranchController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create branch'))
        {
            ...
            $branch->save();
            \DB::commit();
            Utility::makeActivityLog(\Auth::user()->id,'Branch',$branch->id,'Create Branch',$branch->name);

            $html = view('branch.appendrow', ['branches' => [$branch]])->render();
            $data = [
                'datarow' => $html,
                'table_id' => "branch-table",
                'action' => 'add',
                'row_id' => $branch->id,
            ];
            return response()->json(['success' => true, 'message' => __('Branch  successfully created.'), 'data' => $data]);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, Branch $branch)
    {
        ...
        if(\Auth::user()->can('edit branch'))
        {
            if($branch->created_by == \Auth::user()->creatorId())
            {
                ...
                $branch->save();
                \DB::commit();
                Utility::makeActivityLog(\Auth::user()->id,'Branch',$branch->id,'Update Branch',$branch->name);
                $html = view('branch.appendrow', ['branches' => [$branch]])->render();
                $data = [
                    'datarow' => $html,
                    'table_id' => "branch-table",
                ...
                return response()->json(['success' => true, 'message' => __('Branch successfully updated.'), 'data' => $data]);
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
