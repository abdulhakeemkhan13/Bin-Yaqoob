...
class LabelController extends Controller
{
    ...
    public function index()
    {
        if(\Auth::user()->can('manage label'))
        {
            if (\Auth::user()->type == 'company') {
                ...
                $labels   = Label::select('labels.*', 'pipelines.name as pipeline')->join('pipelines', 'pipelines.id', '=', 'labels.pipeline_id')->where('pipelines.owned_by', '=', \Auth::user()->ownedId())->where('labels.owned_by', '=', \Auth::user()->ownedId())->orderBy('labels.pipeline_id')->get();
                $label = Label::where('owned_by',\Auth::user()->ownedId())->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create label'))
        {
            ...
            $label->owned_by = \Auth::user()->ownedId();
            $label->save();
            \DB::commit();
            Utility::makeActivityLog(\Auth::user()->id,'Label',$label->id,'Create Label',$label->name);
            $html = view('labels.appendrow', compact('label'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "label-table",
                'action' => 'add',
                'row_id' => $label->id,
            ];
            return response()->json(['success' => true, 'message' => __('Label successfully created.'), 'data' => $data]);
        }
        else
        {
            return response()->json(['error' => __('Permission Denied.')], 401);
    ...
    public function update(Request $request, Label $label)
    {
        ...
        if(\Auth::user()->can('edit label'))
        {
            ...
            if($label->created_by == \Auth::user()->ownerId())
            {
                ...
                $label->save();
                \DB::commit();
                Utility::makeActivityLog(\Auth::user()->id,'Label',$label->id,'Update Label',$label->name);
                $html = view('labels.appendrow', compact('label'))->render();
                $data = [
                    'datarow' => $html,
                    'table_id' => "label-table",
                ...
                return response()->json(['success' => true, 'message' => __('Label successfully updated.'), 'data' => $data]);
            }
            else
            {
                return response()->json(['error' => __('Permission Denied.')], 401);
    ...
    public function destroy(Label $label)
    {
        if(\Auth::user()->can('delete label'))
        {
            if($label->created_by == \Auth::user()->ownerId())
            {
                Utility::makeActivityLog(\Auth::user()->id,'Label',$label->id,'Delete Label',$label->name);
                $label->delete();
