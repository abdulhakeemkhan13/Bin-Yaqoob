    ...
    public function index()
    {
        if(Auth::user()->can('manage document'))
        {
            if(Auth::user()->type == 'company')
            {
                ...
                $documents = DucumentUpload::where('owned_by', Auth::user()->ownedId())->get();
            }
            else
            {
...
//                    $document->save();
                ...
                $document->save();
                Utility::makeActivityLog(Auth::user()->id,'DocumentUpload',$document->id,'Create DocumentUpload',$document->name);
                DB::commit();
                $documentPath = \App\Models\Utility::get_file('uploads/documentUpload');
                $html = view('documentUpload.appendrow', compact('document', 'documentPath'))->render();
                $data = [
                    'datarow' => $html,
                    'table_id' => "document-upload-table",
                    'action' => 'add',
                    'row_id' => $document->id,
                ];
                return response()->json(['success' => true, 'message' => __('Document successfully created.'), 'data' => $data]);
        }
        else
        {
...
//                                       'document' => 'mimes:jpeg,png,jpg,svg,pdf,doc,zip|max:20480',
                ...
                $document->save();
                Utility::makeActivityLog(Auth::user()->id,'DocumentUpload',$document->id,'Edit DocumentUpload',$document->name);
                DB::commit();
                $documentPath = \App\Models\Utility::get_file('uploads/documentUpload');
                $html = view('documentUpload.appendrow', compact('document', 'documentPath'))->render();
                $data = [
                ...
                return response()->json(['success' => true, 'message' => __('Document successfully updated.'), 'data' => $data]);
            }
            else
            {
    ...
    public function destroy($id)
    {
        if(Auth::user()->can('delete document'))
        {
            ...
            {
                // log will be here/
                Utility::makeActivityLog(Auth::user()->id,'DocumentUpload',$document->id,'Delete DocumentUpload',$document->name);
                $document->delete();
