    ...
    public function index()
    {
        ...
        if($usr->can('manage form builder'))
        {
            if (\Auth::user()->type == 'company') {
                ...
                $forms = FormBuilder::where('owned_by', '=', $usr->ownedId())->get();
            }
        ...
        try {
            if(\Auth::user()->can('create form builder'))
            {
                ...
                if($validator->fails())
                {
                    ...
                    // Don't store complex objects in the session
                ...
                $form_builder->owned_by = \Auth::user()->ownedId();
                $form_builder->save();
                Utility::makeActivityLog(\Auth::user()->id,'Form Builder',$form_builder->id,'Create Form Builder',$form_builder->name);
                \DB::commit();
                $html = view('form_builder.appendrow', compact('form_builder'))->render();
                $data = [
                    'datarow' => $html,
                    'table_id' => "form-builder-table",
                    'action' => 'add',
                    'row_id' => $form_builder->id,
                ];
                return response()->json(['success' => true, 'message' => __('Form successfully created.'), 'data' => $data]);
            }
            else
            {
            ...
            \DB::rollback();
            // Return a JSON response instead of redirecting with a complex error object
        ...
        try {
            ...
            if($usr->can('edit form builder'))
            {
                if($formBuilder->created_by == $usr->creatorId())
                {
                    ...
                    {
                        $messages = $validator->getMessageBag();
                        // Return JSON instead of redirecting with session data
                        return response()->json(['error' => $messages->first()], 422);
                    }
                    ...
                    $formBuilder->save();
                    Utility::makeActivityLog(\Auth::user()->id,'From Builder',$formBuilder->id,'Update FromBuilder',$formBuilder->name);
                    \DB::commit();
                    
                    // Make sure we're using the correct variable name
                    $form_builder = $formBuilder; // Add this line to ensure variable name consistency
                    $html = view('form_builder.appendrow', compact('form_builder'))->render();
                    ...
                    $data = [
                        ...
                        'table_id' => "form-builder-table",
                        'action' => 'edit',
                        'row_id' => $formBuilder->id,
                    ];
                    return response()->json(['success' => true, 'message' => __('Form successfully updated.'), 'data' => $data]);
                }
                else
                {
                    return response()->json(['error' => __('Permission Denied.')], 401);
            ...
            \DB::rollback();
            // Return JSON instead of redirecting with the exception
            return response()->json(['error' => __('An error occurred.'), 'exception' => $e->getMessage()], 500);
        }
    }
    ...
    public function destroy(FormBuilder $formBuilder)
    {
        if(Auth::user()->can('delete form builder'))
        {
            if($formBuilder->created_by == \Auth::user()->ownerId())
            {
                ...
                Utility::makeActivityLog(\Auth::user()->id,'Form Builder',$formBuilder->id,'Create Deal',$formBuilder->name);
                $formBuilder->delete();
