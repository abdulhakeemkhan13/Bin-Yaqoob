...
class AttendanceEmployeeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $branch = Branch::where($user->type == 'company' ? 'created_by' : 'owned_by', $user->type == 'company' ? $creatorId : $ownedId)->pluck('name', 'id');
        $branch->prepend('Select Branch', '');

        $department = Department::where($user->type == 'company' ? 'created_by' : 'owned_by', $user->type == 'company' ? $creatorId : $ownedId)->pluck('name', 'id');
        ...
        if ($user->type != 'client' && $user->type != 'company') {
            ...
            $employee = Employee::select('id')->where($user->type == 'company' ? 'created_by' : 'owned_by', $user->type == 'company' ? $creatorId : $ownedId);
    ...
    public function create()
    {
        if (\Auth::user()->can('create attendance')) {
            $employees = User::where('created_by', '=', Auth::user()->creatorId())->where('type', '=', "employee")->where(function ($query) {
        ...
        if (\Auth::user()->can('create attendance')) {
            ...
            if ($attendance) {
                ...
                if ($employeeAttendance->employee_id) {
                    $emp = Employee::find($employeeAttendance->employee_id);
                    $emp->termination_date = $employeeAttendance->date;
                    $emp->save();
                }
                Utility::makeActivityLogged(\Auth::user()->id, 'AttendanceEmployee', $employeeAttendance->id, 'delete AttendanceEmployee', $employeeAttendance->employee->name);
    ...
    public function edit($id)
    {
        ...
        $employees = Employee::where($user->type == 'company' ? 'created_by' : 'owned_by', $user->type == 'company' ? $creatorId : $ownedId)->where(function ($query) {
            $query->whereNull('termination_date')
                ->orWhere('termination_date', '>', now()->endOfMonth());
        })->get()->pluck('name', 'id');
    ...
    public function update(Request $request, $id)
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'HR') {
            ...
            if ($check->date == date('Y-m-d')) {
                ...
                if ($check->employee_id) {
                    $emp = Employee::find($check->employee_id);
                    $emp->termination_date = $check->date;
                    $emp->save();
                }
                Utility::makeActivityLogged(\Auth::user()->id, 'AttendanceEmployee', $employeeId->id, 'delete AttendanceEmployee', $employeeId->employee->name);
        ...
        if (Auth::user()->type == 'Employee') {
            ...
            Utility::makeActivityLogged(\Auth::user()->id, 'AttendanceEmployee', $attendanceEmployee->id, 'delete AttendanceEmployee', $attendanceEmployee->employee->name);
    ...
    public function destroy($id)
    {
        if (\Auth::user()->can('delete attendance')) {
            ...
            if ($attendance) {
                Utility::makeActivityLogged(\Auth::user()->id, 'AttendanceEmployee', $attendance->id, 'delete AttendanceEmployee', $attendance->employee->name);
            }
            $attendance->delete();
    ...
    public function attendance(Request $request)
    {
        ...
        if (empty($checkDb)) {
            ...
            Utility::makeActivityLog(\Auth::user()->id, 'Employee Attendance', $employeeAttendance->id, 'Create Employee Attendance', $employeeAttendance->status);
    ...
    public function bulkAttendance(Request $request)
    {
        ...
        // if($request->date == ''){
        //     return redirect()->back()->with('error', __('Date is required.'));
        // }
        $user = \Auth::user();
        $creatorId = $user->creatorId();
        $ownedId = $user->ownedId();
        ...
        $department = Department::where($user->type == 'company' ? 'created_by' : 'owned_by', $user->type == 'company' ? $creatorId : $ownedId)->pluck('name', 'id');
        $department->prepend('Select Department', '');
        $employeesQuery = Employee::where($user->type == 'company' ? 'created_by' : 'owned_by', $user->type == 'company' ? $creatorId : $ownedId);
        if (!empty($request->branch)) {
        ...
        $employees = $employeesQuery->where(function ($query) {
            $query->whereNull('termination_date')
                ->orWhere('termination_date', '>', now()->endOfMonth());
        })->get();
...
public function bulkAttendanceData(Request $request)
{
    if (\Auth::user()->can('create attendance')) {
        ...
        if ($validator->fails()) {
            // send the first validation message to your toaster
        ...
        if (!empty($employees)) {
            foreach ($employees as $employee) {
                ...
                if ($request->$present === 'on') {
                    $in  = date("H:i:s", strtotime($request->$inKey));
                    $out = date("H:i:s", strtotime($request->$outKey));
                    ...
                    $attendance = AttendanceEmployee::where('employee_id', $employee)
                        ->where('date', $date)->first();

                    $employeeAttendance = $attendance ?? new AttendanceEmployee();
                    if (!$attendance) {
                        $employeeAttendance->employee_id = $employee;
                        $employeeAttendance->created_by  = \Auth::user()->creatorId();
                        $employeeAttendance->owned_by    = \Auth::user()->ownedId();
                    }
                    ...
                    $employeeAttendance->late          = $late;
                    $employeeAttendance->early_leaving = ($totalEarlyLeavingSeconds > 0) ? $earlyLeaving : '00:00:00';
                    $employeeAttendance->overtime      = $overtime;
                    $employeeAttendance->total_rest    = '00:00:00';
                    $employeeAttendance->save();
