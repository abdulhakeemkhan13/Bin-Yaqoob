...
class ClientController extends Controller
{
    ...
    public function index()
    {
        ...
        {
            $user    = \Auth::user();
            $clientsquery = User::where('type', '=', 'client');
            if (\Auth::user()->type == 'company') {
                $clientsquery->where('created_by', '=', $user->creatorId());
            } else {
                $clientsquery->where('owned_by', '=', $user->ownedId());
            }
        ...
        if(\Auth::user()->can('create client'))
        {
            ...
            if($total_client < $plan->max_clients || $plan->max_clients == -1)
            {
                ...
                if($setings['new_client'] == 1)
                {
                    ...
                    Utility::makeActivityLog(\Auth::user()->id,'Client',$client->id,'Create Client',$client->name);
                    \DB::commit();
                    $cardHtml = view('clients.appendrow', compact('client'))->render();
                    $data = [
                        'card_html' => $cardHtml,
                        'card_container' => "clients-cards",
                        'action' => 'add',
                        'card_id' => $client->id,
                    ];
                    return response()->json(['success' => true, 'message' => __('Client successfully created.'), 'data' => $data]);
                }
                Utility::makeActivityLog(\Auth::user()->id,'Client',$client->id,'Create Client',$client->name);
                \DB::commit();
                $cardHtml = view('clients.appendrow', compact('client'))->render();
    ...
    public function update(User $client, Request $request)
    {
        ...
        if(\Auth::user()->can('edit client'))
        {
            ...
            if($client->created_by == $user->creatorId())
            {
                ...
                Utility::makeActivityLog(\Auth::user()->id,'Client',$client->id,'Update Client',$client->name);
                \DB::commit();
                $cardHtml = view('clients.appendrow', compact('client'))->render();
                $data = [
                    'card_html' => $cardHtml,
                ...
                return response()->json(['success' => true, 'message' => __('Client successfully updated.'), 'data' => $data]);
            ...
            if($client->created_by == $user->creatorId())
            {
                ...
                if(empty($estimation))
                {
                  /*  ClientDeal::where('client_id', '=', $client->id)->delete();
                    ...
                    Utility::makeActivityLog(\Auth::user()->id,'Client',$client->id,'Delete Client',$client->name);
                    $client->delete();
    ...
    public function clientPasswordReset(Request $request, $id)
    {
        ...
        \DB::commit();
        $cardHtml = view('clients.appendrow', ['client' => $user])->render();
        $data = [
            'card_html' => $cardHtml,
            'card_container' => "clients-cards",
            'action' => 'edit',
        ...
        return response()->json(['success' => true, 'message' => __('Client Password successfully updated.'), 'data' => $data]);
