    ...
    public function store(Request $request)
    {
        ...
        try {
            if (\Auth::user()->can('create training type')) {
                ...
                // Pass the variable as 'trainingtype' (singular) to match what the view expects
                $html = view('trainingtype.appendrow', compact('trainingtype'))->render();
                $data = [
                    'datarow' => $html,
                    'table_id' => "trainingtype-table",
                    'action' => 'add',
                    'row_id' => $trainingtype->id,
                ];
                return response()->json(['success' => true, 'message' => __('Training Type successfully created.'), 'data' => $data]);
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } catch (\Exception $e) {
        ...
        try {
            if (\Auth::user()->can('edit training type')) {
                ...
                if ($trainingType->created_by == \Auth::user()->creatorId() || $trainingType->owned_by == \Auth::user()->ownedId()) {
                    ...
                    // Pass the variable as 'trainingtype' (singular) to match what the view expects
                    $trainingtype = $trainingType; // Rename to match view expectation
                    $html = view('trainingtype.appendrow', compact('trainingtype'))->render();
                    $data = [
                        'datarow' => $html,
                        'table_id' => "trainingtype-table",
                    ...
                    return response()->json(['success' => true, 'message' => __('Training Type successfully updated.'), 'data' => $data]);
                } else {
                    return redirect()->back()->with('error', __('Permission denied.'));
                }
            } else {
