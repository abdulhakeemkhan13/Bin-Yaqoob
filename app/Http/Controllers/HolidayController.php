...
class HolidayController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (\Auth::user()->can('manage holiday')) {
            ...
            $holidays = Holiday::where($column, '=', $ownerId);
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create holiday')) {
            ...
            \DB::commit();
            $employeeIds = [];
            $branchEmployeeIds = Employee::where('branch_id', \Auth::user()->creatorId())
                ->pluck('id')
                ->toArray();
            ...
            foreach ($userarr as $key => $notifyto) {
                Utility::makeNotification($notifyto, 'holiday', $dataarr, $holiday->id, 'Announce a ' . $holiday->occasion . ' holiday', \Auth::user()->name);
            }
            //For Notification
            ...
            if ($webhook) {
                ...
                if ($status == true) {
                    Utility::makeActivityLog(\Auth::user()->id, 'Holiday', $holiday->id, 'Create Holiday', $holiday->occasion);
                } else {
                    return redirect()->back()->with('error', __('Webhook call failed.'));
                }
            }
            Utility::makeActivityLog(\Auth::user()->id, 'Holiday', $holiday->id, 'Create Holiday', $holiday->occasion);
            $html = view('holiday.appendrow', compact('holiday'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "holiday-table",
                'action' => 'add',
                'row_id' => $holiday->id,
            ];
            return response()->json(['success' => true, 'message' => __('Holiday successfully created.'), 'data' => $data]);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, Holiday $holiday)
    {
        ...
        if (\Auth::user()->can('edit holiday')) {
            ...
            \DB::commit();
            // log
            Utility::makeActivityLog(\Auth::user()->id, 'Holiday', $holiday->id, 'Update Holiday', $holiday->occasion);
            $html = view('holiday.appendrow', compact('holiday'))->render();
            $data = [
                'datarow' => $html,
                'table_id' => "holiday-table",
            ...
            return response()->json(['success' => true, 'message' => __('Holiday successfully updated.'), 'data' => $data]);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        } catch (\Exception $e) {
    ...
    public function destroy(Holiday $holiday)
    {
        if (\Auth::user()->can('delete holiday')) {
            // log
            Utility::makeActivityLog(\Auth::user()->id, 'Holiday', $holiday->id, 'Delete Holiday', $holiday->occasion);
            $holiday->delete();
