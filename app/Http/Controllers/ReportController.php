...
class ReportController extends Controller
{
    ...
    public function invoiceSummary(Request $request)
    {
        ...
        if (\Auth::user()->can('invoice report')) {
            ...
            if ($request->ajax()) {
                return view('report.invoice_report', compact(
                    'invoices',
                    'invoiceTotal',
                    'totalInvoice',
                    'totalDueInvoice',
            ...
            // Regular request - return the full view
            return view('report.invoice_report', compact(
                'invoices',
                'invoiceTotal',
                'totalInvoice',
    ...
    public function accountStatement(Request $request)
    {
        ...
        return redirect()->back()
            ->with('error', 'Start Month cannot be greater than End Month.');
    }
                $start = strtotime($request->start_month);
                $end = strtotime($request->end_month);
            } else {
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => true,
                    'html' => [
                        'printableArea' => view('report.statement_report_printable', compact('reportData', 'filter'))->render(),
                        'tableRows' => view('report.statement_report_rows', compact('reportData'))->render()
                    ]
                ]);
            }
    ...
    public function chequeListReport(Request $request)
    {
        ...
        // Base: checklist + relations we’ll display from
        $q = CheckList::query()
            ->with([
                'payment.bankAccount',     // for revenue-side cheques
                'billPayment.bankAccount', // for payment-side cheques
                'bankAccount',             // direct fallback
            ])
            ->where('created_by', \Auth::user()->creatorId())
            ->whereBetween('date', [$start->toDateString(), $end->toDateString()]);

        // Account filter (matches check_lists.account_id)
        if ($request->filled('account')) {
            $q->where('account_id', $request->account);
        }

        // Cheque number filter (partial match)
        ...
        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'html' => [
                    'printableArea' => view('report.statement_report_printable', compact('rows', 'filter'))->render(),
                    'tableRows'     => view('report.statement_report_rows', compact('rows'))->render(),
                ],
            ]);
        }

        return view('report.chequeList_report', compact('rows', 'account', 'filter'));
    }
    ...
    public function payroll(Request $request)
    {
        ...
        if (\Auth::user()->can('manage report')) {
            ...
            foreach ($payslips as $payslip) {
                ...
                $saturationDeductions = $payslip->saturation_deduction;
                foreach ($saturationDeductions as $saturationDeduction) {
                    $totalSaturationDeduction += $saturationDeduction->amount;
                }

                $otherPayments = $payslip->other_payment;
